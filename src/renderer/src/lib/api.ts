...
export const api = {
  // ── 研究输入 ──────────────────────────────
  /** 提交研究输入，返回生成的卡片 ID */
  submitResearch: (payload: SubmitResearchInput) =>
    invoke<{ cardId: string }>(IPC.RESEARCH_SUBMIT, payload),

  /** 读取研究卡片详情 */
  getResearchCard: (cardId: string) =>
    invoke<ResearchCard | null>(IPC.RESEARCH_GET_CARD, cardId),

  // ── 论文项目 ──────────────────────────────
  /** 从研究卡片创建论文项目 */
  createProjectFromCard: (payload: CreateProjectFromCard) =>
  ...
  /** 为项目选择模板 */
  setTemplate: (projectId: string, templateId: string) =>
