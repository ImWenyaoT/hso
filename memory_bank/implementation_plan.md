...
**目标：** 构建 HSO 基础产品：用户可以输入研究起点，获得结构化研究卡片，将其转化为论文项目，组织最简论文结构，运行 LaTeX 构建，并在桌面应用中预览或导出生成的 PDF。
...
以下决策已在基础产品阶段锁定，除非产品方向发生变化，否则实施过程中不得重新讨论：
...
- [ ] **步骤 1：定义最小代码库结构**
记录基础产品的初始应用和服务布局：Electron 桌面 shell、渲染进程 UI、应用/编排层、LaTeX worker、共享 schema/契约以及文档。将 TypeScript 作为所有运行时拥有代码路径的默认实现语言。
...
**目标成果：** 基础产品数据模型足够稳定，能够支撑完整的第一版用户循环。

- [ ] **步骤 1：定义最小实体集**
记录基础产品所需的最小实体：`research_card`、`paper_project`、`paper_section`、`template`、`asset`、`reference_source`、`build_job`、`build_artifact` 和 `error_event`。将 `user` 视为后续扩展的可选基础设施，而非 v1 必需的领域对象。

测试：
- 检查每个实体是否直接支持基础产品流程中的至少一个步骤。
- 移除任何不支持第一版用户操作的实体。
